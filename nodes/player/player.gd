...
@export var WALL_JUMP_FORCE = 210
...
func _p(k: float, w: float = 1.0, _exp: float = POWER_EXP) -> float:
	return k * pow(max(POWER * w, 0.0), _exp)
...
func _integrate_forces(state: PhysicsDirectBodyState2D) -> void:
	...
	if _hover_disabled:
		...
		if _result:
			...
			# corner-stall fix: if grounded, don't treat as wall-grab (restore full gravity)
			_is_grabbing_wall = false
			_gravity_force = state.get_total_gravity()
			...
			var _damping_force_h = -HOVER_DAMPING * _velocity_along_ray
			var _total_hover_force = (_spring_force + _damping_force_h) * _ray_direction * HOVER_FORCE_MULTIPLIER
	...
	var _horizontal_speed = absf(_current_velocity.x)
	var _factor = clamp(1.0 - (_horizontal_speed / eff_max_speed()), 0.0, 1.0)
	...
	if _on_ground:
		# If reversing direction, give more authority to the input-side force (your original idea).
		...
		# Use horizontal velocity directly for damping (F = -c * v_x), not normalized * total speed.
		var h := Vector2(_current_velocity.x, 0.0)
		var _damping_force = -h * DAMPING_COEFFICIENT * _damping_factor
		state.apply_central_force(_damping_force)
	# -----------------------------------

	# Slide along contact, but keep the intended drive magnitude along the tangent (corner-stall fix)
	if _normal.length_squared() > 0:
		var _pre_force = _applied_force
		_applied_force = _applied_force.slide(_normal)
		if _pre_force.length() > 0.001 and _applied_force.length() < _pre_force.length():
			_applied_force = _applied_force.normalized() * _pre_force.length()

	state.apply_central_force(_applied_force)

	if _on_ground and _move_len == 0 and abs(_current_velocity.x) > 0.01:
		var h2 := Vector2(_current_velocity.x, 0.0)
		var _damping_force2 = -h2 * DAMPING_COEFFICIENT
		state.apply_central_force(_damping_force2)
	if abs(_current_velocity.x) < 0.001:
		var v_snap := state.get_linear_velocity()
		v_snap.x = 0.0
		state.set_linear_velocity(v_snap)
...
func _draw() -> void:
	...
	# Drive/applied force (red)
	draw_arrow(Vector2.ZERO, _applied_force * 0.3, Color(1, 0, 0))
	# Gravity (blue)
	draw_arrow(Vector2.ZERO, _gravity_force * 0.1, Color(0, 0, 1))
	# Contact normal (green)
	draw_arrow(Vector2.ZERO, _normal * 40, Color(0, 1, 0))

	# Damping force
	var h_last := Vector2(_velocity_last_frame.x, 0.0)
	var _damping_force_dbg = -h_last * DAMPING_COEFFICIENT
	draw_arrow(Vector2.ZERO, _damping_force_dbg * 0.1, Color(1, 0.5, 0))
